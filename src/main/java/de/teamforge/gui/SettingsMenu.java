    ...
    protected void build() {
        ...
        setItem(10, ItemBuilder.of(Material.IRON_SWORD)
                ...
                .lore(plugin.getMessages().getList("gui.settings.ff.lore",
                        Placeholder.component("state", plugin.getMessages().onOffComponent(team.isFriendlyFire()))))
        ...
        setItem(12, ItemBuilder.of(Material.OAK_FENCE_GATE)
                ...
                .lore(plugin.getMessages().getList("gui.settings.open.lore",
                        Placeholder.component("state", plugin.getMessages().onOffComponent(team.isOpen()))))
                .build(), e -> {
        ...
        if (plugin.getGlowService().isActive()) {
            setItem(28, ItemBuilder.of(Material.GLOWSTONE_DUST)
                    .name(plugin.getMessages().get("gui.settings.glow.name"))
                    .lore(plugin.getMessages().getList("gui.settings.glow.lore",
                            Placeholder.component("state", plugin.getMessages().onOffComponent(team.isGlow()))))
                    .glow(team.isGlow()).build(), e -> {
                if (!plugin.getTeamManager().canEditSettings(team, viewer.getUniqueId())) {
                    errorSound();
                    plugin.getMessages().send(viewer, "error.no-permission");
                    return;
                }
                team.setGlow(!team.isGlow());
                plugin.getTeamManager().markDirty();
                plugin.getGlowService().refreshTeam(team);
                clickSound();
                plugin.getMessages().send(viewer, team.isGlow() ? "glow.enabled" : "glow.disabled");
                refresh();
            });
        }
